...
def tts_with_cloned_voice(text: str, reference_audio_path: str):
    ''' 克隆pipelline,各种参数也是在这里面调
         params: 
            text:需要克隆语音读的文本内容
            reference_audio_path:需要克隆的音色的原文件路径
         '''
    # tts_text = "make xiaoniuma great again"# 也就是让它说的话
    mode_checkbox_group = '自然语言控制'# 可选 自然语言控制 预训练音色 跨语种复刻 
    sft_dropdown = refresh_sft_spk()['choices'][7] # 控制选择的预训练音色
    prompt_text = ""# reference_audio中说的话的内容 3s极速复刻模式下必须有
    prompt_wav_upload = None
    # prompt_wav_record = 'audios/麦克阿瑟.mp3'
    instruct_text = "用粤语说这句话"# 也就是对它的要求，自然语言控制 3s极速复刻模式下必须有
    # instruct_text = None
    wav_path="output/tts_clone_预训练音色.wav"
    seed = 42
    stream = False
    speed = 1.0
    generator = generate_audio(text, mode_checkbox_group, sft_dropdown,
                           prompt_text, prompt_wav_upload, reference_audio_path,
...
if __name__ == '__main__':
    tts_with_cloned_voice("还得是小牛马",'audios/麦克阿瑟.mp3')